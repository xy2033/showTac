...
class _LLMNameDict(dict):
    """A dict that detects LLM type from model path, supporting both HF Hub names and local paths."""
    _FUZZY_MAP = [
        ('qwen', 'qwen2_5'),
        ('llama', 'llama3'),
    ]
    ...
    def __getitem__(self, key):
        ...
        # Then try fuzzy match based on path content
        key_lower = key.lower()
        for pattern, name in self._FUZZY_MAP:
            if pattern in key_lower:
                return name
        raise KeyError(f"Cannot determine LLM type from path: {key}. "
                       f"Known mappings: {dict(self)}")


path_to_llm_name = _LLMNameDict({
