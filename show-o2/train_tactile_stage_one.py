...
def main():
    ...
    # Wan2.1 VAE encodes videos causally in temporal chunks: first frame, then
    # groups of four frames. For example, 5 input frames become 2 latent frames,
    # while 17 input frames become 5 latent frames. The token layout must match
    # the latent temporal length, not the raw input frame count.
    ...
    # For time embedding: prepend one time token to each video segment.
    ...
    def prepare_latents_and_labels(
        ...
        For tactile_visual_data:
            - pixel_values contains (B*2, C, num_frames, H, W) clips
              Even segments are visual clips, odd segments are tactile clips
            - Visual video segment: t=1.0 (clean), flow loss masked
            - Tactile video segment: noised via transport, flow loss computed

        Returns:
            image_latents: (B*2, C, T_latent, H_vae, W_vae) — per-video latents for model
            t: (B*2,) — one timestep per video segment
            image_labels: (B*2, C, T_latent, H_vae, W_vae) — velocity targets (ut)
            recons_images: reconstructed images from VAE
            image_masks: modified masks (visual segment zeroed out)
        """
        # VAE encode: pixel_values shape (B*2, C, num_frames, H, W).
        # Keep the temporal dimension so WanVAE can use its 3D causal encoder.
        if config.model.vae_model.type == 'wan21':
            if len(pixel_values.shape) == 4:
                pixel_values = pixel_values.unsqueeze(2)
            image_latents = vae_model.sample(pixel_values)  # (B*2, 16, T_latent, Hv, Wv)
        ...
        b, num_segments = shape  # b=batch_size, num_segments=2 (visual + tactile)
        ...
        c, latent_frames, h, w = image_latents.shape[1:]  # 16, T_latent, Hv, Wv
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for batch in mixed_loader:
            ...
            # Flatten to (B*2, C, num_frames, H, W) for clip-level 3D Causal VAE encoding
            b, m, num_frames_t = pixel_values.shape[:3]
            pixel_values = rearrange(pixel_values, 'b m t c h w -> (b m) c t h w')
            # data_type is repeated per video segment
            batch['data_type'] = batch['data_type'] * m
...
def visualize_reconstruction_tactile(
    ...
    if pixel_values.dim() == 5:
        # pixel_values/recons_images: (B*2, C, T, H, W) after clip-level VAE path.
        b = pixel_values.shape[0] // 2
        pixel_values = pixel_values.reshape(b, 2, *pixel_values.shape[1:])
        recons_images = recons_images.reshape(b, 2, *recons_images.shape[1:])

        vis_orig = denorm_vid(pixel_values[0, 0].unsqueeze(0))
        vis_recon = denorm_vid(recons_images[0, 0].unsqueeze(0))
        tac_orig = denorm_vid(pixel_values[0, 1].unsqueeze(0))
        tac_recon = denorm_vid(recons_images[0, 1].unsqueeze(0))
    else:
        # Backward-compatible per-frame path.
    ...
    try:
        ...
        if target_pixel_values.dim() == 5:
            ...
            target_pixel_values = target_pixel_values.reshape(
                batch_size, 2, *target_pixel_values.shape[1:]
            )
